...
int hashfunction(char *name) {
    ...
    int ln = strlen(name);
    for (int i = 0; i < ln; i++) {
        hashval = (hashval + ((*(name + i) - '0' + 1) % HASHTABLE_SIZE) * pow) %
                  HASHTABLE_SIZE;
...
void htInsert(HTABLE table, node_item newItem) {
    int key = hashfunction(newItem.name);
    NODE newNode = createNewNode(newItem);
    if (!table->items[key])
        table->items[key] = createNewList();
    insertNodeFirst(newNode, table->items[key]);
}

void freeList(LL list) { free(list); }
