...
char *getLexeme(char *begin, char *forward) {
    ...
    if ((begin <= (buf1 + BUFFER_SIZE + 1) &&
        ...
        char *res = (char *)malloc(sizeof(char) * (size_t)(forward - begin));

        size_t subtract = forward - begin;
        ...
        if (begin <= (buf1 + BUFFER_SIZE + 1) &&
            ...
            char *temp1 = (char *)malloc(sizeof(char) * (size_t)(temp - begin));
            size_t substract1 = temp - begin;
...
void failure() {
    if (strlen(getLexeme(lexemeBegin, forward)) == 0) {
        printf("Line No %d: ", lineNumber);
        printf("Error : Unknown symbol <");
        printf("%c", *lexemeBegin);
        printf(">\n");
    } else if (strlen(getLexeme(lexemeBegin, forward)) == 1) {
        printf("Line No %d: ", lineNumber);
        printf("Error : Unknown symbol <");
